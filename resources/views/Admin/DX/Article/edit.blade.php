                                ...
                                <div class="col-sm-6">
                                    <select class="form-control" name="column_id">
                                        ...
                                        @foreach($data['column'] as $column)
                                            <option value={{$column['id']}} {{$data['article']->column_id == $column['id'] ? 'selected' : ''}}>{{$column['title']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                        ...
                                        @foreach($data['label'] as $value)
                                            <tr>
                                                @foreach($value as $label)
                                                    <td style="width: 23%"> <input type="checkbox" name="labels[]"  style="width: 20px; display: inline-block; margin-bottom: 6px;"  value="{{$label['id']}}" {{in_array($label['id'],array_keys($lables)) ? 'checked' : ''}} onclick="oneChoice()"> {{$label['name']}}: <input type="number"  style="width: 60px; display: inline-block; margin-bottom: 6px;"  min="0.0" max="1.0" value="{{in_array($label['id'],array_keys($lables)) ? $lables[$label['id']] : '0.0'}}" name="ranks" step="0.1" onclick="oneChoice()"></td>
                                                @endforeach
                                            </tr>
                                        @endforeach
                                    </table>
